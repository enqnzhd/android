...
/**
 * TBXML for Java Transcription from excellent XML parser TBXML from Tom Bradley
 *
 * @author julien.foltz
 */
public class TBXML {
    ...
    public class TBXMLElement {
        ...
        public TBXMLElement parentElement = null, firstChild = null,
                currentChild = null, nextSibling = null,
    ...
    public TBXML(URI uri) throws TBXMLException, MalformedURLException,
            IOException {
        this(uri.toURL());
    }
    ...
    public String valueOfAttributeForElement(String aName,
                                             TBXMLElement aXMLElement) {
        ...
        while (attribute != null) {
            // TODO: create a function that compares 2 Strings, inside TBXML,
    ...
    public TBXMLElement childElement(String aName,
                                     TBXMLElement aParentXMLElement) {
    ...
    // TODO: \0 values are written at a bad place, spaces often appear after the
    // texts.
    private void decodeBytes() throws TBXMLException {
        ...
        while ((elementStart = strstr(elementStart, "<")) >= 0) {
            ...
            // if cdata section found, skip data within cdata section and remove
            // cdata tags
            if (isCDATA == 0) {
                ...
                // find start of next element skipping any cdata sections within
                // text
                ...
                memcpy(elementStart, elementStart + 9, CDATAEnd - elementStart
                        - 9);
            ...
            // null terminate element start so previous element text doesn't
            // overrun
            ...
            if (bytes[elementNameStart] == '?'
                    || (bytes[elementNameStart] == '!' && isCDATA != 0)) {
            ...
            if (bytes[elementNameStart] == '/') {
                ...
                if (parentXMLElement != null) {
                    ...
                    if (parentXMLElement.text >= 0) {
                        ...
                        int end = parentXMLElement.text
                                + strlen(parentXMLElement.text) - 1;
                        while (end > parentXMLElement.text
                                && isspace(bytes[end]))
                            bytes[end--] = 0;
                    }
                    ...
                    if (parentXMLElement != null
                            && parentXMLElement.firstChild != null)
            ...
            // element may contain no attributes and would return nil while
            // looking for element name end
            // <tile>
                ...
                while (chr++ < elementEnd) {
                    ...
                    switch (mode) {
                        ...
                        case TBXML_ATTRIBUTE_VALUE_END:
                            if (bytes[chr] == '<'
                                    && strncmp(chr, "<![CDATA[", 9) == 0) {
                                mode = TBXML_ATTRIBUTE_CDATA_END;
                            } else if ((bytes[chr] == '"' && singleQuote == false)
                                    || (bytes[chr] == '\'' && singleQuote == true)) {
                                bytes[chr] = 0;
                                ...
                                while ((CDATAStart = strstr(value, "<![CDATA[")) >= 0) {
                                    ...
                                    memcpy(CDATAStart, CDATAStart + 9,
                                            strlen(CDATAStart) - 8);
                                ...
                                // if this is the first attribute found, set pointer
                                // to this attribute on element
                                if (xmlElement.firstAttribute == null)
                                    xmlElement.firstAttribute = xmlAttribute;
                                // if previous attribute found, link this attribute
                                // to previous one
                                if (lastXMLAttribute != null)
    ...
    private int strstr_s(int index, String s) throws TBXMLException {
        ...
        if (result < 0)
            throw new TBXMLException(s + " not found, but should be");
        return result;
    }
